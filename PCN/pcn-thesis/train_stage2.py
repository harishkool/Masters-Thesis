...
from util import load_training_data, save_pcd, load_training_data_stage2
import pdb
import shutil
...
def train(args):
    ...
    coarse_pl = tf.placeholder(tf.float32, (args.batch_size, 1024, 3), 'inputs')
    features_pl = tf.placeholder(tf.float32, (args.batch_size, 1024))
    npts_pl = tf.placeholder(tf.int32, (None), 'num_points')
    gt_pl = tf.placeholder(tf.float32, (args.batch_size, 4096, 3), 'ground_truths')
    ...
    for epoch in range(args.num_epochs):
        ...
        for btch in range(nm_btchs):
            ...
            # pdb.set_trace()           
            inputs, gt, features, ids = load_training_data_stage2(btch_indx, training_list, args.lmdb_train)
            ids_g = ids
            # pdb.set_trace()
            start = time.time()
            ...
            feed_dict = {coarse_pl: inputs, features_pl:features, gt_pl: gt, is_training_pl: True} #add voxels also here
        ...
        if epoch % args.steps_per_eval == 0:
            ...
            for i in range(num_eval_steps):
                ...
                feed_dict = {coarse_pl: inputs, features_pl:features, gt_pl: gt, is_training_pl: False} #add voxels also here
...
if __name__ == '__main__':
    ...
    parser.add_argument('--steps_per_visu', type=int, default=3)
