...
def train(args):
    ...
    model = model_module.Model(inputs_pl, gt_pl, alpha)
    ...
    for epoch in range(args.num_epochs):
        ...
        for btch in range(nm_btchs):
            ...
            # pdb.set_trace()
            inputs, gt, _, _, voxels, ids = load_training_data(btch_indx, training_list, args.lmdb_train) 
            start = time.time()
            feed_dict = {inputs_pl: inputs, gt_pl: gt, is_training_pl: True} #add voxels also here
        ...
        if epoch % args.steps_per_eval == 0:
            ...
            for i in range(num_eval_steps):
                ...
                inputs, gt, _, _, voxels, ids = load_training_data(btch_indx, valid_list, args.lmdb_valid)
                feed_dict = {inputs_pl: inputs, gt_pl: gt, is_training_pl: False} #add voxels also here
            ...
            print(colored('epoch %d  step %d  loss %.8f - time per batch %.4f' %
                        (epoch, step, total_eval_loss / total_seen, total_time / num_eval_steps),
                        'grey', 'on_green'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log_dir', default='log/pcn_original')
