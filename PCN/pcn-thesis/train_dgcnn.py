...
def train(args):
    ...
    alpha = tf.train.piecewise_constant(global_step, [6000, 12000, 30000],
                                        [0.01, 0.1, 0.5, 1.0], 'alpha_op')
    beta = tf.train.piecewise_constant(global_step, [10000, 15000, 40000],
    ...
    for epoch in range(args.num_epochs):
        ...
        for btch in range(nm_btchs):
            ...
            feed_dict = {inputs_pl1: inputs, inputs_pl2: inputs_2, inputs_pl3: inputs_3, inputs_pl4: inputs_4,
                    coarse_pl:coarse, middle_pl: middle, voxels_pl:voxels, gt_pl: gt, is_training_pl: True} #add voxels also here
            # feed_dict = {inputs_pl: inputs, gt_pl: gt, is_training_pl: True}
            _, loss, _, step, summary = sess.run([train_op, model.loss, batch, global_step, train_summary], feed_dict=feed_dict)
            total_loss  += loss
        ...
        if epoch % args.steps_per_eval == 0:
            ...
            print(colored('epoch %d  step %d  loss %.8f - time per batch %.4f' %
                        (epoch, step, total_eval_loss / num_valid, total_time / num_eval_steps),
                        'grey', 'on_green'))
