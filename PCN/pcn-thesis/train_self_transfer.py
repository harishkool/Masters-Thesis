...
def train(args):
    ...
    gt_pl = tf.placeholder(tf.int32, ((args.batch_size)), 'ground_truths')
    ...
    final_layer = graph.get_tensor_by_name("agg/weights:0")
    maxpool = tf.reduce_max(final_layer, axis=2, keep_dims=True)
    maxpool = tf.reshape(maxpool,(args.batch_size,-1))
    ...
    for epoch in range(args.num_epochs):
        ...
        for btch in range(nm_btchs):
            ...
            # pdb.set_trace()
            gt = np.zeros((args.batch_size))
            feed_dict = {inputs_pl: inputs, gt_pl: gt, is_training_pl: True} 
            start = time.time()
            max_out = sess.run([maxpool], feed_dict=feed_dict)
            # _, loss_val, step, _, summary = sess.run([train_op, loss, global_step, batch, train_summary], feed_dict=feed_dict)
            pdb.set_trace()
            # total_loss  += loss_val
