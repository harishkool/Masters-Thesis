...
def train(args):
    ...
    beta = tf.train.piecewise_constant(global_step, [10000, 20000, 50000],
                                        [0.01, 0.1, 0.5, 1.0], 'alpha_op')
    inputs_pl = tf.placeholder(tf.float32, (args.batch_size, None, 3), 'inputs')
    # voxels_pl = tf.placeholder(tf.float32, (args.batch_size, None, None, 3))
    ...
    for step in range(init_step+1, args.max_step+1):
        ...
        start = time.time()
        # feed_dict = {inputs_pl: inputs, voxels_pl:voxels, gt_pl: gt, is_training_pl: True} #add voxels also here
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lmdb_train', default='/shared/kgcoe-research/mil/harish/pcn_data/data/shapenet/train.lmdb')
    parser.add_argument('--lmdb_valid', default='/shared/kgcoe-research/mil/harish/pcn_data/data/shapenet/valid.lmdb')
    parser.add_argument('--log_dir', default='log/pcn_capsule')
    parser.add_argument('--model_type', default='pcn_cd_novoxel')
    ...
    parser.add_argument('--steps_per_eval', type=int, default=6000)
