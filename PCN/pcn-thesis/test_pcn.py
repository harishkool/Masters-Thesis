...
# Total loss 46.04855424 loss 0.06616172
# Total loss 6.86633263 loss 0.00986542 -- only complete point clouds
# Total loss 45.43261013 loss 0.06490373 -- EMD distance
# Total loss 39.42141798 loss 0.05631631  -- Coarse loss EMD
...
def test(args):
    ...
    coarse_pl = tf.placeholder(tf.float32, (args.batch_size, 1024, 3), 'coarse_ground_truths')

    config = tf.ConfigProto()
    config.gpu_options.allow_growth = True
    config.allow_soft_placement = True
    model_module = importlib.import_module('.%s' % args.model_type, 'models')
    model = model_module.Model(inputs_pl,  gt_pl, coarse_pl, alpha)
    pdb.set_trace()
    sess = tf.Session(config=config)
    saver = tf.train.Saver()
    saver.restore(sess, args.log_dir)
    ...
    for i in range(num_eval_steps):
        ...
        feed_dict = {inputs_pl: inputs, coarse_pl:coarse,  gt_pl: gt, is_training_pl: False} 
