...
def train(args):
    ...
    beta = tf.train.piecewise_constant(global_step, [15000, 25000, 50000],
                                        [0.01, 0.1, 0.5, 1.0], 'alpha_op')
    ...
    model = model_module.Model(inputs_pl, gt_pl, voxels_pl, coarse_pl, middle_pl, alpha, beta, is_training_pl, bn_decay=bn_decay)
    ...
    best_dir = os.path.join(args.log_dir, 'best_model')
    ...
    for epoch in range(args.num_epochs):
        ...
        for btch in range(nm_btchs):
            ...
            start = time.time()
            feed_dict = {inputs_pl: inputs, voxels_pl:voxels, gt_pl: gt, coarse_pl:coarse, middle_pl:middle, 
                    is_training_pl: True} #add voxels also here
        ...
        if epoch % args.steps_per_eval == 0:
            ...
            for i in range(num_eval_steps):
                ...
                feed_dict = {inputs_pl: inputs, voxels_pl:voxels, gt_pl: gt, coarse_pl:coarse, middle_pl:middle, 
                    is_training_pl: False} #add voxels also here
            ...
            if(eval_loss < best_eval):
                best_eval = eval_loss
                bst_dir = os.path.join(args.log_dir, 'best_model')
                saver.save(sess, os.path.join(bst_dir, 'model'), step)  
