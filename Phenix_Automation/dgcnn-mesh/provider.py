...
def get_meshfiles(directr):
  dir_list = os.listdir(directr)
  return dir_list


def loadmeshFile(dir,mode='train'):
  fc_fl = loadmat(os.path.join('./data_mesh',mode, dir, 'faces.mat'))
  vc_fl = loadmat(os.path.join('./data_mesh',mode, dir, 'vertices.mat'))
  fc_vx = loadmat(os.path.join('./data_mesh',mode, dir, 'facevertxdata.mat'))
  faces = fc_fl['fces']
  ...
  for indx, face in enumerate(faces):
      ...
      vrtx3 = list(vertices[face[2]])
      # features = vrtx1 + vrtx2 + vrtx3
      x_mean  = (vrtx1[0] + vrtx2[0] + vrtx3[0])/3
      y_mean  = (vrtx1[1] + vrtx2[1] + vrtx3[1])/3
      z_mean  = (vrtx1[2] + vrtx2[2] + vrtx3[2])/3
      inputs.append([x_mean, y_mean, z_mean])
      if all_labels[indx][0]==1:
          gt.append(0)
      elif all_labels[indx][1] or all_labels[indx][2]==1:
          gt.append(1)
